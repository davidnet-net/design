...
function extractContent(content) {
	...
	const metaMatch = content.match(/<meta\s+name=["']description["']\s+content=["'](.*?)["']\s*\/?>/i);
	if (metaMatch) {
		description = metaMatch[1].trim();
	}

	// Headings
	const headings = [...content.matchAll(/<h[1-6][^>]*>(.*?)<\/h[1-6]>/gi)].map((m) => m[1].trim());

	// Paragraphs
	const paragraphs = [...content.matchAll(/<p[^>]*>(.*?)<\/p>/gi)].map((m) => m[1].replace(/<[^>]+>/g, "").trim());
...
const index = files.map((file) => {
	...
	const slug = file.replace(CONTENT_DIR, "").replace("/+page.svelte", "") || "/";
